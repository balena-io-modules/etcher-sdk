 ...
 */
async function openBootDrive(
	driveLetter: string,
	partition: TargetPartition,
	device: TargetDevice,
) {
	// Device must be online to write file.
	partition.volumeId = await diskpart.findVolume(
		device.name,
...
export const enum MigrateResult {
	OK,
	ERROR,
}
...
export const migrate = async (
	...
	try {
		...
		const bootPartition: TargetPartition = {
			volumeId: '',
			driveLetter: '',
		};
		const rootAPartition: TargetPartition = {
			volumeId: '',
			driveLetter: '',
		};
		...
		bootPartition.etcher = await findFilesystemLabel(
			etcherPartitions,
			targetDevice.etcher,
			BOOT_PARTITION_LABEL,
			'fat16',
		...
		if (bootPartition.etcher) {
			console.log(
				`Boot partition already exists at index ${bootPartition.etcher.index}`,
			);
			bootPartition.volumeId = await diskpart.findVolume(
				targetDevice.name,
				BOOT_PARTITION_LABEL,
			);
		...
		rootAPartition.etcher = await findFilesystemLabel(
			etcherPartitions,
			targetDevice.etcher,
		...
		if (rootAPartition.etcher) {
			console.log(
				`RootA partition already exists at index ${rootAPartition.etcher.index}`,
			);
			rootAPartition.volumeId = await diskpart.findVolume(
				targetDevice.name,
				ROOTA_PARTITION_LABEL,
			);
		...
		if (requiredFreeSize) {
			const unallocSpace =
				(await diskpart.getUnallocatedSize(targetDevice.name)) * 1024;
			console.log(
				`Found ${unallocSpace} (${formatMB(
					unallocSpace,
				)} MB) not allocated on disk ${targetDevice.name}`,
			);
		...
		if (tasks.includes('copy')) {
			...
			if (!bootPartition.etcher) {
				...
				const afterFirstPartitions =
					await targetDevice.etcher.getPartitionTable();
				...
				console.log(
					`Created new partition for boot at offset ${bootPartition.etcher.offset} with size ${bootPartition.etcher.size}`,
				);
			...
			if (!rootAPartition.etcher) {
				...
				console.log(
					`Created new partition for data at offset ${rootAPartition.etcher.offset} with size ${rootAPartition.etcher.size}`,
				);
			...
			try {
				await copyBootloaderFromImage(
					...
					`${bootPartition.driveLetter}:${BOOT_FILES_TARGET_PATH}`,
				);
			} finally {
				// Just leave boot volume open if config task will write to it as well below.
				if (!tasks.includes('config')) {
					await diskpart.clearDriveLetter(
						bootPartition.volumeId,
